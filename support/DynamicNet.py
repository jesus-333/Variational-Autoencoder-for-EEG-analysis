...
from support_DynamicNet import getActivationList, getPoolingList, convOutputShape, LinearCombinationForMatrix
...
class DynamicCNN(nn.Module):
    ...
    def __init__(self, parameters, print_var = False, tracking_input_dimension = False):
        ...
        # Set if add a flatten layer (ONLY IF layer_ff == 0)
        if("add_flatten_layer" in parameters.keys() and layers_ff == 0): self.add_flatten_layer = parameters["add_flatten_layer"] 
        ...
        if("activation_list" in parameters.keys()): 
            ...
            if(self.add_flatten_layer): 
                ...
                if(len(activation_list) != layers_cnn): raise Exception("wrong number of elements in activation_list")
        ...
        if("dropout_list" in parameters.keys()): 
            ...
            if(self.add_flatten_layer): 
                ...
                if(len(dropout_list) != layers_cnn): raise Exception("wrong number of elements in dropout_list")
        ...
        if("bias_list" in parameters.keys()): 
            ...
            if(self.add_flatten_layer): 
                ...
                if(len(bias_list) != layers_cnn): raise Exception("wrong number of elements in bias_list")
    ...
    def gradCAM(self, x, idx_class = -1, device = torch.device("cpu")):
        if(self.multi_block_structure):
            # Move all to device
            self.to(device)
            x.to(device)
            
            # Pass to evaluation mode
            self.eval()
            
            # Get activation before softmax
            softmax_depth = self.getDepth() - 1
            y = self.getMiddleResults(x, softmax_depth).requires_grad_(True)
            
            # Clean gradients
            x.requires_grad_(True)
            self.zero_grad()
            
            # Check eventual index of the class
            if(idx_class == -1): idx_class = np.argmax(y.cpu().detach().numpy())
            
            # Obtain activation value for the choosen index
            y_one_hot = np.zeros((1, y.size()[-1]), dtype=np.float32)
            y_one_hot[0][idx_class] = 1
            y_one_hot = torch.from_numpy(y_one_hot).requires_grad_(True)
            y_one_hot = torch.sum(y_one_hot * y)
            
            # Backward step
            y_one_hot.backward(retain_graph=True)
            
            print(y)
            print(y_one_hot)
        else:
            print("Method supported only for network with multi_block_structure = True")
