...
for i in range(len(subj_list)):
    ...
    train_data = test_dataset.data.squeeze()
    test_data = train_dataset.data.squeeze()

    for j in range(len(ch_list)):
        idx_ch_source = ch_list[j] == ch_list
        source_data = train_data[:, idx_ch_source, :].flatten()
        print("\t{} - {} SOURCE (TRAIN) channel".format(j, ch_list[j]))
        
        for k in range(len(ch_list)):
            
            idx_ch_target = ch_list[k] == ch_list
            target_data = test_data[:, idx_ch_target, :].flatten()
            print("\t\t{} - {} TARGET (TEST) channel".format(k, ch_list[k]))
